resource "aws_security_group" "chef_server" {
  ...
  tags {
    Name      = "${var.tag_automate}_chef_server_security_group_${random_id.automate_instance_id.hex}"
    X-Dept    = "${var.tag_dept}"
...
resource "aws_security_group" "chef_automate" {
  ...
  tags {
    Name      = "${var.tag_automate}_chef_automate_security_group"
    X-Dept    = "${var.tag_dept}"
    X-Contact = "${var.tag_contact}"
  }
}
...
resource "aws_security_group" "build_nodes" {
  ...
  tags {
    Name      = "${var.tag_automate}_build_nodes_security_group"
    X-Dept    = "${var.tag_dept}"
    X-Contact = "${var.tag_contact}"
  }
}
